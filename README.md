...
- Introduced `mt-doctor` system diagnostics script (`03-doctor.sh`) to streamline environment health checks and troubleshooting.
- Implemented framework version validation using local cached metadata.
- Added sync configuration verification for `SYNC_REPO_URL`, GitHub CLI authentication, and local Git remote targeting.
- Added deep Git state inspections for the sync repository, detecting in-progress merges, uncommitted changes, unpushed commits, and open or stale PR branches.
- Added config schema checking to identify legacy `config.yaml` keys pending migration.
